...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  # languages[language_name] = languages[language_name].merge(Hash[info_key, info_value])
  languages
end
...
def find_beautiful_languages(languages)
  ...
  languages = languages.filter { |language, language_info_value| language_info_value[:is_beautiful?].eql?(true) }
  languages
end
